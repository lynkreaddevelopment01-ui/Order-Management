...
function downloadOrderPDF(id) {
  window.open(`${API_BASE}/reports/order-pdf/${id}?token=${getToken()}`, '_blank');
}
function downloadOrderExcel(id) {
  window.open(`${API_BASE}/reports/order-excel/${id}?token=${getToken()}`, '_blank');
}
